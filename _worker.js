...
// Variables
const rootDomain = "amzarmuhammad87.workers.dev"; // Ganti dengan domain utama kalian
const serviceName = "backup"; // Ganti dengan nama workers kalian
const apiKey = "03c5e4872e9d687aff013f323e6d24c446f6d"; // Ganti dengan Global API key kalian (https://dash.cloudflare.com/profile/api-tokens)
const apiEmail = "amzarmuhammad87@gmail.com"; // Ganti dengan email yang kalian gunakan
...
function getAllConfig(request, hostName, proxyList, page = 0) {
    ...
    try {
        ...
        document.setTitle("Welcome to <span class='text-blue-500 font-semibold'>Amzar VPN 2</span>");
                        ...
                        for (const port of filterPort) {
                            for (const protocol of filterVPN) {
                                ...
                                if (protocol == "ss") {
                                    ...
                                    uri.searchParams.set(
                                        "plugin",
                                        `v2ray-plugin${port == 80 ? "" : ";tls"};mux=0;mode=websocket;path=/${proxy.proxyIP}-${proxy.proxyPort
                                        };host=${APP_DOMAIN}`
                                    );
                                } else {
                                    uri.username = uuid;
...
class Document {
    ...
    buildProxyGroup() {
        ...
        // Fixed: Added proper container with max-width and better responsive grid
        proxyGroupElement += `<div class="max-w-7xl mx-auto px-4">`;
        proxyGroupElement += `<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">`;
        for (let i = 0; i < this.proxies.length; i++) {
            const proxyData = this.proxies[i];

            // Assign proxies
            proxyGroupElement += `<div class="lozad scale-95 mb-4 bg-white dark:bg-slate-800 transition-all duration-300 rounded-lg p-4 md:p-6 flex flex-col shadow-md hover:shadow-lg border border-slate-200 dark:border-slate-700 hover:scale-105">`;
            proxyGroupElement += `  <div id="countryFlag" class="absolute -translate-y-9 md:-translate-y-11 -translate-x-2 border-4 border-white dark:border-slate-800 rounded-full overflow-hidden"><img width="40" class="md:w-12" src="https://hatscripts.github.io/circle-flags/flags/${proxyData.country.toLowerCase()}.svg" /></div>`;
            ...
            proxyGroupElement += `  <div class="rounded-lg py-3 md:py-4 px-3 md:px-4 bg-slate-50 dark:bg-slate-700/50 flex-grow mt-4">`;
            proxyGroupElement += `    <h5 class="font-bold text-base md:text-lg text-slate-800 dark:text-slate-100 mb-1 overflow-x-scroll scrollbar-hide text-nowrap">${proxyData.org}</h5>`;
            ...
            proxyGroupElement += `    </div>`;
            proxyGroupElement += `  </div>`;
            proxyGroupElement += `  <div class="flex flex-col gap-2 mt-4 text-xs md:text-sm">`;
            for (let x = 0; x < proxyData.list.length; x++) {
                ...
                if (x % 2 == 0) {
                    proxyGroupElement += `<div class="flex gap-1 md:gap-2 justify-around w-full">`;
                }

                proxyGroupElement += `<button class="bg-indigo-500 hover:bg-indigo-600 dark:bg-indigo-500 dark:hover:bg-indigo-400 rounded-md p-1.5 md:p-2 w-full text-white font-semibold transition-colors duration-200 text-xs md:text-sm" onclick="copyToClipboard('${proxy}')">${indexName[x]}</button>`;
    ...
    addPageButton(text, link, isDisabled) {
        const pageButton = `<li><button ${isDisabled ? "disabled" : ""
